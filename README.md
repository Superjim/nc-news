...
| GET /api/articles                       | Retrieves a collection of all available articles from the API. <br />The default query parameters include 'sort_by' (created_at), 'order' (descending), and 'topic' (all topics), 'limit' (10) and 'p' (1). <br />Additional query options include 'sort_by' (such as article_id, title, topic, author, body, created_at, votes), 'order' (ascending or descending), 'limit' (a number between 1 and 50), 'p' (the page number) and 'topic' (the slug of a specific topic). <br />It is also possible to filter articles by multiple topics by separating them with a comma in the query string. |
...
| POST /api/articles                      | Posts a new article to the database. <br />The 'author' must already exist in the database (username). <br />The 'topic' must already exist in the database. <br />The 'article_img_url' can be left empty and the default one will be used. If a custom 'article_img_url' is used it must be a valid, hosted image with a GET status 200. <br />The 'body' and 'topic' must not be empty.                                                                                                                                                                                                     |
